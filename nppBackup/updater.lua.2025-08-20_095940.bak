...
-- Стиль ImGui с улучшенным дизайном
local style = {}
function style.blue(imgui)
    imgui.OnInitialize(function()
        ...
        colors[clr.PopupBg] = ImVec4(0.12, 0.12, 0.14, 0.90)
        colors[clr.Border] = ImVec4(0.20, 0.20, 0.25, 0.50)
        ...
        colors[clr.TitleBg] = ImVec4(0.08, 0.08, 0.10, 1.00)
        colors[clr.TitleBgActive] = ImVec4(0.15, 0.30, 0.50, 1.00)
        colors[clr.TitleBgCollapsed] = ImVec4(0.08, 0.08, 0.10, 0.75)
        colors[clr.MenuBarBg] = ImVec4(0.12, 0.12, 0.14, 1.00)
        colors[clr.ScrollbarBg] = ImVec4(0.05, 0.05, 0.07, 0.50)
        ...
        colors[clr.Header] = ImVec4(0.20, 0.40, 0.70, 0.50)
        colors[clr.HeaderHovered] = ImVec4(0.25, 0.50, 0.80, 0.80)
        colors[clr.HeaderActive] = ImVec4(0.30, 0.60, 0.90, 1.00)
        colors[clr.Separator] = ImVec4(0.20, 0.20, 0.25, 0.50)
        colors[clr.SeparatorHovered] = ImVec4(0.25, 0.50, 0.80, 0.78)
        colors[clr.SeparatorActive] = ImVec4(0.30, 0.60, 0.90, 1.00)
        colors[clr.ResizeGrip] = ImVec4(0.20, 0.40, 0.70, 0.25)
        colors[clr.ResizeGripHovered] = ImVec4(0.25, 0.50, 0.80, 0.67)
        colors[clr.ResizeGripActive] = ImVec4(0.30, 0.60, 0.90, 0.95)
        colors[clr.CloseButton] = ImVec4(0.50, 0.50, 0.50, 0.50)
        colors[clr.CloseButtonHovered] = ImVec4(0.90, 0.40, 0.40, 1.00)
        colors[clr.CloseButtonActive] = ImVec4(0.90, 0.40, 0.40, 1.00)
        colors[clr.PlotLines] = ImVec4(0.60, 0.60, 0.60, 1.00)
        colors[clr.PlotLinesHovered] = ImVec4(0.90, 0.50, 0.40, 1.00)
        colors[clr.PlotHistogram] = ImVec4(0.80, 0.60, 0.00, 1.00)
        colors[clr.PlotHistogramHovered] = ImVec4(0.90, 0.70, 0.00, 1.00)
        colors[clr.TextSelectedBg] = ImVec4(0.25, 0.50, 0.80, 0.35)
...
function module.set(imgui, name)
    if name == nil or style[name] == nil then return false end
    style[name](imgui)
    return true
end
...
function loadJson(file_path, default_data)
    ...
    if file then
        ...
        logToFile("Файл JSON не найден: " .. file_path)
    end
    return default_data
end
...
function saveJson(file_path, data)
    ...
    if file then
        ...
        logToFile("Ошибка сохранения JSON: " .. file_path)
    end
end
...
function processItems(items, local_data, category, callback)
    ...
    for _, entry in ipairs(items or {}) do
        if not entry.name or not entry.version or not entry.path then
            ...
            if isNewerVersion(local_ver, entry.version) then
                ...
                elseif success then
                    new_data.versions[entry.name] = entry.version
                    ...
                    logToFile("Обновлен " .. category .. " " .. entry.name .. " до версии " .. entry.version)
                    sampAddChatMessage("[Updater] Обновлен " .. category .. " " .. entry.name .. " до версии " .. entry.version, 0x00FF00)
                    updated = true
                    ...
                    if entry.run_after_download then
                        ...
                        local???? = file_path:match("^(.*\\)[^\\]+$") or SAVE_PATH
                        local success, err = pcall(function()
                ...
                new_data.versions[entry.name] = local_data[entry.name] or entry.version
...
function downloadUpdates()
    ...
    if not remote_versions or type(remote_versions) ~= "table" then
        logToFile("remote_versions не определить")
    ...
    for _, entry in ipairs(remote_versions.scripts or {}) do
        if entry.name then
            enabled_scripts[entry.name] = imgui.new.bool(enabled_scripts[entry.name] and enabled_scripts[entry.name][0] or true)
        end
    end
    if development_versions and development_versions.scripts and isPlayerAllowed() then
...
function checkForUpdates()
    ...
    if doesFileExist(analytics1) then pcall(os.remove, DEV_TEMP_JSON) end
    if not success then
    ...
    local function load_or_unload_script(entry, is_dev)
        ...
        if action == "load" and not script_handle then
            if not enabled_scripts[entry.name] then
                enabled_scripts[entry.name] = imgui.new.bool(true)
                saveJson(ENABLED_JSON, enabled_scripts)
            end
        ...
        if success then
            ...
            if is_script then
                ...
                if not enabled_scripts[name] then
                    enabled_scripts[name] = imgui.new.bool(true)
                    saveJson(ENABLED_JSON, enabled_scripts)
                end
    ...
    if items then
        for _, entry in ipairs(items) do
            ...
            if is_script then
                ...
                if imgui.Checkbox(u8("##enabled_" .. name), enabled_scripts[name] or imgui.new.bool(true)) then
                    enabled_scripts[name] = enabled_scripts[name] or imgui.new.bool(true)
                    saveJson(ENABLED_JSON, enabled_scripts)
...
imgui.OnFrame(
    ...
    function()
        ...
        local success, err = pcall(function()
            imgui.Begin(u8"Dunaevskiy Lua Scripts##main_window", showMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
            imgui.BeginGroup()
            local button_width = isPlayerAllowed() and 174 or 232
