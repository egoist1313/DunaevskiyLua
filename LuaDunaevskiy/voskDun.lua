...
local remote_versions = nil
local download_progress = 0
local total_files = 0
local downloaded_files = 0
local update_status = "Checking for updates..."
...
function downloadFileAsync(url, save_path, callback)
    ...
    downloadUrlToFile(url, save_path, function(id, status, p1, p2)
        if status == dlstatus.STATUSEX_ENDDOWNLOAD then
            ...
            download_progress = math.floor((downloaded_files / total_files) * 100)
            callback(true)
...
function downloadUpdates()
    lua_thread.create(function()
        ...
        if not remote_versions or type(remote_versions) ~= "table" then
            logToFile("Ошибка: remote_versions не определено или не является таблицей")
            sampAddChatMessage("[Vosk] Нет данных для обновления файлов", 0xFF0000)
            update_status = "No update data available"
            return
        end
        downloaded_files = 0
        ...
        if updated_files then
            update_status = "Files updated successfully"
       elder: sampAddChatMessage("[Vosk] Пожалуйста, загрузите необходимые файлы: https://github.com/egoist1313/DunaevskiyLua", 0xFF0000)
        else
            update_status = "All files are up to date"
        end
...
-- Rest of the original script functions (mergeTables, setupMimgui, saveSettings, loadSettings, etc.)
...
function main()
    ...
    checkForUpdates()
    lua_thread.create(function()
        while not remote_versions do wait(100) end
        downloadUpdates()
        if update_status == "All files are up to date" or update_status == "Files updated successfully" then
            ...
            sampAddChatMessage('[Vosk] Пожалуйста, загрузите необходимые файлы: https://github.com/egoist1313/DunaevskiyLua', 0xFF0000)
            return
        end
    end)
    imgui.OnFrame(
        ...
        function()
            ...
            if imgui.Button(u8"Сбросить настройки по умолчанию") then
                ...
                fonts.indicators = createFont(ini.fonts.indicators.name, ini.fonts.indicators.size, ini.fonts.indicators.flags)
                fonts.clipboardText = createFont(ini.fonts.clipboardText.name, ini.fonts.clipboardText.size, ini.fonts.clipboardText.flags)
                sampAddChatMessage('[Vosk] Настройки сб sudden', 0x71c8dd)
            end
            imgui.End()
